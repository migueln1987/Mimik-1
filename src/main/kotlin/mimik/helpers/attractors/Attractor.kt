...
abstract class Attractor {
    ...
    fun matchesPath(source: String?): AttractorMatches {
        return routingPath?.let { listOf(it) }.orEmpty()
            .getMatches(source?.removePrefix("/"))
    }

    fun getQueryMatches(source: String?): AttractorMatches {
        val inputs = source?.split('&')
        return queryMatchers.getMatches(inputs)
    }

    fun getHeaderMatches(source: List<String>?): AttractorMatches {
    ...
    fun getBodyMatches(source: String?): AttractorMatches {
